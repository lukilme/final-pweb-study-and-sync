...
export class UserValidator {
  ...
  static registerValidate(data: UserRegisterData) {
    ...
    this.passwordUserValidate(
      data.passwordRegisterField,
      data.repeat_passwordRegisterField
    );
  ...
  private static nameUserValidate(name: string) {
    if (!name || name.length < this.MIN_NAME || name.length > this.MAX_NAME) {
      throw new Error(
        `Name must be between ${this.MIN_NAME} and ${this.MAX_NAME} characters.`
      );
    }
  ...
  private static emailUserValidate(email: string) {
    if (
      !email ||
      email.length < this.MIN_EMAIL ||
      email.length > this.MAX_EMAIL
    ) {
      throw new Error(
        `Email must be between ${this.MIN_EMAIL} and ${this.MAX_EMAIL} characters.`
      );
    }
   ...
   */
  private static passwordUserValidate(
    password: string,
    repeat_password: string
  ) {
    if (!password || password.length < this.MIN_PASSWORD) {
      throw new Error(
        `Password must be at least ${this.MIN_PASSWORD} characters long.`
      );
    }
